		...
		<div class="over_lay_agile_pages_w3ls">
				<div class="registration">
					...
					<div class="signin-form profile">
						<h2>Entrer votre code de validation</h2>
						<div class="login-form">
							@if(session()->has('ok'))
								<div class="alert alert-danger alert-dismissible">{!! session('ok') !!}</div>
							@endif
							<form  method="post" action="{{ route('codeValidationForm') }}">
								@csrf
								<div class="form-group {!! $errors->has('code') ? 'has-error' : '' !!}">
									<input type="text" name="code" placeholder="Code de validation" required="">
									{!! $errors->first('code', '<small class="help-block">:message</small>') !!}
                                </div>
                                @if(session()->has('email'))
                                    <div class="form-group">
                                        <p>Votre code de validation à été envoyé à l'adresse suivant : <b>{!! session('email') !!}</b></p>
                                    </div>
                                @endif
								<div class="tp">
